#!/bin/bash
#
# AIPC Installation Script (Sub2API-compatible runtime)
# AIPC 安装脚本（运行时保留 Sub2API 兼容标识）
# Usage: curl -sSL https://raw.githubusercontent.com/HeartEase1/aipc/main/deploy/install.sh | bash
#
...
# Configuration
# External release source. Keep the runtime paths/service names below stable
# so existing Sub2API installations can upgrade in place without data moves.
GITHUB_REPO="HeartEase1/aipc"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="AIPC 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 AIPC 服务："
    ...
    ["not_installed"]="AIPC 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 AIPC。"
    ...
    ["server_config_desc"]="配置 AIPC 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="AIPC Installation Script"
    ...
    # Completion
    ["install_complete"]="AIPC installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start AIPC service:"
    ...
    ["not_installed"]="AIPC is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove AIPC from your system."
    ...
    ["uninstall_complete"]="AIPC has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install AIPC"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove AIPC"
    ...
    ["server_config_desc"]="Configure AIPC server listen address"
...
[Unit]
Description=AIPC - Sub2API-compatible AI API Gateway Platform
Documentation=https://github.com/HeartEase1/aipc
