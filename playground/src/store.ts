...
import {
  getPlaygroundStorageName,
  isHostedMode,
  protectHostedExportOptions,
  protectHostedImportOptions,
  protectHostedRuntimeSettings,
  scrubHostedSettings,
} from './lib/hosted'
...
// Hosted workspaces may expose local preferences and local data management,
// but their API configuration remains controlled by the parent application.
...
export function getPersistedState(state: AppState) {
  ...
  if (!isHostedMode) return persisted

  return {
    ...persisted,
    settings: scrubHostedSettings(persisted.settings),
...
function mergePersistedState(persistedState: unknown, currentState: AppState): AppState {
  ...
  return {
    ...currentState,
    ...plan.state,
    settings: protectHostedRuntimeSettings(
      currentState.settings,
      scrubHostedSettings(plan.state.settings),
    ),
    dismissedCodexCliPrompts: isHostedMode ? [] : plan.state.dismissedCodexCliPrompts,
...
interface AppState {
  ...
  setHostedSettings: (s: AppSettings) => void
  ...
  persist(
    (set, get) => ({
      ...
      setSettings: (s) => set((st) => {
        ...
        const settings = protectHostedRuntimeSettings(previous, normalizeSettings(merged))
      ...
      setShowSettings: (showSettings, settingsTabRequest) => {
        ...
        const safeTabRequest = isHostedMode
          && settingsTabRequest
          && !HOSTED_SETTINGS_TABS.has(settingsTabRequest)
          ? 'general'
          : settingsTabRequest
        set({
          showSettings,
          ...(safeTabRequest ? { settingsTabRequest: safeTabRequest } : {}),
...
async function executeAgentRound(
  ...
  try {
    ...
    const maxToolCalls = Number.isFinite(requestSettings.agentMaxToolRounds)
      ...
      : DEFAULT_AGENT_MAX_TOOL_ROUNDS
    const maxAutomaticRequests = isHostedMode ? Math.min(maxToolCalls, 6) : maxToolCalls
    ...
    let responseRequestsUsed = 0
    let imageRequestsUsed = resume?.toolCallsUsed ?? 0
    ...
    const executeSingleImageFunctionCall = async (functionCallItem: ResponsesOutputItem): Promise<string | null> => {
      ...
      if (imageRequestsUsed >= maxAutomaticRequests) {
        return JSON.stringify({ id: item.id, status: 'error', error: `已达到单次 Agent 最多 ${maxAutomaticRequests} 个生图请求的限制` })
      }
      imageRequestsUsed += 1
    ...
    const executeBatchFunctionCall = async (functionCallItem: ResponsesOutputItem): Promise<string> => {
      ...
      const remainingImageRequests = maxAutomaticRequests - imageRequestsUsed
      if (batchItems.length > remainingImageRequests) {
        return JSON.stringify({
          error: `本批次包含 ${batchItems.length} 个生图请求，超过本次 Agent 剩余的 ${Math.max(remainingImageRequests, 0)} 个请求额度`,
          images: batchItems.map((item) => ({
            id: item.id,
            status: 'error',
            error: '已达到单次 Agent 自动请求限制',
          })),
        })
      }
      imageRequestsUsed += batchItems.length
    ...
    while (true) {
      ...
      if (responseRequestsUsed >= maxAutomaticRequests) {
        reachedToolLimit = true
        break
      }
      responseRequestsUsed += 1
      ...
      if (
        ...
        || responseRequestsUsed >= maxAutomaticRequests
        || imageRequestsUsed >= maxAutomaticRequests
      ) {
        reachedToolLimit = true
        break
      }
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    const effectiveOptions = protectHostedExportOptions(options)
    if (!effectiveOptions.exportConfig && !effectiveOptions.exportTasks) {
      throw new Error(isHostedMode ? '站内工作台不导出 API 配置。' : '请至少选择一项要导出的数据。')
    }
    const state = useStore.getState()
    if (effectiveOptions.exportTasks && hasActiveDataOperations(state.tasks, state.agentConversations)) throw new Error('当前有任务正在进行，请完成或停止后再导出。')
    const tasks = effectiveOptions.exportTasks ? await getAllTasks() : []
    const imageIds = effectiveOptions.exportTasks ? await getAllImageIds() : []
    const { settings, agentConversations, favoriteCollections, defaultFavoriteCollectionId } = state
    const exportedAt = Date.now()
    const params = {
      options: effectiveOptions,
      exportedAt,
      settings: effectiveOptions.exportConfig
        ? scrubHostedSettings(settings)
        : scrubHostedSettings(DEFAULT_SETTINGS),
      tasks,
      imageTasks: tasks,
      favoriteCollections,
      defaultFavoriteCollectionId,
      agentConversations: effectiveOptions.exportTasks ? getPersistableAgentConversations(agentConversations) : [],
    }
    const imageSizes = []
...
export async function importData(input: File | File[], options: ImportOptions = { importConfig: true, importTasks: true }): Promise<boolean> {
  try {
    const effectiveOptions = protectHostedImportOptions(options)
    if (!effectiveOptions.importConfig && !effectiveOptions.importTasks) {
      throw new Error(isHostedMode ? '站内工作台不导入 API 配置。' : '请至少选择一项要导入的数据。')
    }
    const state = useStore.getState()
    if (effectiveOptions.importTasks && hasActiveDataOperations(state.tasks, state.agentConversations)) throw new Error('当前有任务正在进行，请完成或停止后再导入。')
    ...
    for (const file of files) {
      const manifest = await readExportZipManifest(new Uint8Array(await file.arrayBuffer()), effectiveOptions.importTasks)
    ...
    if (multipart) {
      ...
      if (effectiveOptions.importTasks && (selected.length !== first.total || indexes.size !== first.total)) {
    ...
    const settingsManifests = effectiveOptions.importConfig
      ? selected.filter((part) => part.manifest.settings)
      : []
    if (effectiveOptions.importConfig && !effectiveOptions.importTasks && !settingsManifests.length) throw new Error('所选备份不包含配置数据。')
    ...
    if (effectiveOptions.importTasks && hasTaskData) {
    ...
    if (effectiveOptions.importConfig && settingsManifests.length) {
      const state = useStore.getState()
    ...
    } else if (effectiveOptions.importConfig && settingsManifests.length) {
      msg = '配置已成功导入'
    }
