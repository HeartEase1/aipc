...
describe('agent built-in image tool failure', () => {
  ...
  it('stops Agent continuation loops even when function calls never consume the success budget', async () => {
    ...
    vi.mocked(callAgentResponsesApi).mockResolvedValue({
      ...
      outputItems: [{
        ...
        call_id: 'continue-without-image',
        arguments: JSON.stringify({ reason: 'retry' }),
      }],
    ...
    expect(useStore.getState().agentConversations[0].messages.find((message) => message.role === 'assistant')?.content)
      .toContain('已达到最大工具调用次数（2）')
  })

  it('rejects a Hybrid image batch that exceeds the remaining automatic request budget', async () => {
    const imageProfile = createDefaultOpenAIProfile({ id: 'image-profile', apiKey: 'image-key', apiMode: 'images' })
    useStore.setState({
      settings: normalizeSettings({
        ...useStore.getState().settings,
    ...
    vi.mocked(callAgentResponsesApi)
      .mockResolvedValueOnce({
        ...
        responseId: 'response-oversized-batch',
      })
      .mockResolvedValueOnce({
        text: '已停止批量生图',
        images: [],
        outputItems: [{ type: 'message', content: [{ type: 'output_text', text: '已停止批量生图' }] }],
        responseId: 'response-after-limit',
      })
    ...
    expect(callAgentResponsesApi).toHaveBeenCalledTimes(2)
    expect(JSON.stringify(vi.mocked(callAgentResponsesApi).mock.calls[1][0].input)).toContain('超过本次 Agent 剩余的 2 个请求额度')
  })

  it('aborts an in-flight Hybrid image request when Agent generation is stopped', async () => {
    const imageProfile = createDefaultOpenAIProfile({ id: 'image-profile', apiKey: 'image-key', apiMode: 'images' })
    ...
    vi.mocked(callAgentResponsesApi).mockResolvedValueOnce({
      ...
      outputItems: [{
        type: 'function_call',
        name: 'generate_image',
        call_id: 'hybrid-stop-call',
        arguments: JSON.stringify({ id: 'single', prompt: 'single prompt' }),
      }],
      responseId: 'response-function',
    })
    vi.mocked(callImageApi).mockImplementationOnce((opts) => {
      imageSignal = opts.signal
      return new Promise((_resolve, reject) => {
        const abort = () => reject(imageSignal?.reason ?? new DOMException('Agent 请求已停止', 'AbortError'))
        if (imageSignal?.aborted) abort()
        else imageSignal?.addEventListener('abort', abort, { once: true })
      })
    })
    ...
    await vi.waitFor(() => expect(useStore.getState().agentConversations[0].rounds[0]).toMatchObject({
      status: 'error',
      error: '已停止生成。',
    }))
  })
