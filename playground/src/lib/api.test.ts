...
describe('callImageApi', () => {
  ...
  it.each(['images', 'responses'] as const)('aborts an in-flight %s request when the caller stops it', async (apiMode) => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((_input, init) => {
      ...
      return new Promise((_resolve, reject) => {
        const abort = () => reject(requestSignal.reason ?? new DOMException('请求已停止', 'AbortError'))
