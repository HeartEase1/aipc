...
[最新リリース](https://github.com/HeartEase1/aipc/releases/latest) · [ソース](https://github.com/HeartEase1/aipc) · [上流 Sub2API](https://github.com/Wei-Shaw/sub2api)

**独立メンテナンスされる Sub2API フォーク：AI API の配分と管理を拡張**
...
AIPC は [Sub2API](https://github.com/Wei-Shaw/sub2api) を基にした独立したコミュニティメンテナンス版です。上流プロジェクトとの所属関係、認可、公式な推奨はありません。上流のアーキテクチャとライセンスについては元プロジェクトを参照してください。

上流互換の API ゲートウェイを維持しながら、実運用に役立つ管理・ユーザー向け機能を追加しています。

- **モダンコンソール** - モダン UI と従来コンソールを切り替え可能。ルート、権限、API 契約、業務ロジックは共有します。
- **オンラインワークベンチ** - ログイン後、サイトの API キーでブラウザから対話と画像生成を利用できます。
- **ランキングと給付** - 使用量、支出、紹介報酬のランキング、および管理者による福利・補償の監査可能な記録。
- **キャンペーンとアクセス制御** - 期間限定割引、ランキング参加、地域アクセス方針を管理画面で設定。
- **運用可視化** - 使用量、キャッシュヒット率、リクエストごとの出力速度、チャネル監視、操作監査。
- **選択的な互換性修正** - OpenAI 互換リクエスト、アカウントスケジューリング、フェイルオーバー、各プロバイダーの修正を確認のうえ取り込み。

### メンテナンスと互換性

- 上流の変更はレビューして選択的に取り込みます。各リリースは AIPC のリリースであり、公式 Sub2API のリリースを意味しません。
- フォーク固有の動作は Release ノートとガイドに記載します。API 互換性、課金境界、データベース移行は個別に確認します。
- 更新前にデータベースと設定をバックアップし、Release ノートを確認してから、デプロイ環境のリリース経路で更新してください。


## 概要

AIPC は、AI 製品のサブスクリプションから API クォータを配分・管理するために設計された AI API ゲートウェイプラットフォームです。ユーザーはプラットフォームが生成した API キーを通じて上流の AI サービスにアクセスでき、プラットフォームは認証、課金、負荷分散、リクエスト転送を処理します。
...
- **モダン/クラシックの 2 コンソール** - AIPC のモダン UI と従来の Sub2API スタイルを切り替え可能
- **オンラインワークベンチ** - 設定済みの API キーでブラウザから対話と画像生成
- **ランキングと給付** - 使用量・支出・紹介報酬のランキング、福利・補償の記録
- **期間限定割引** - 条件に合う残高リクエストへ時間帯別の割引倍率を適用
- **使用量インサイト** - キャッシュヒット率、リクエストごとの出力速度、詳細ログ、チャネル監視
...
> **AIPC デプロイに関する注意:** インストーラー、Docker 準備スクリプト、既定のアプリケーションイメージは AIPC のリリースチャネルを使用します。既存環境は WebUI または [AIPC Releases](https://github.com/HeartEase1/aipc/releases) から更新できます。互換性のため、`sub2api.service`、`/opt/sub2api`、`sub2api` コンテナ名は維持されます。

### 公式 Sub2API から AIPC への移行

このフォークは既存のデータベーススキーマ、設定パス、サービス名、データボリュームを維持するため、既存環境上で切り替えられます。先にバックアップを取り、移行中に既存サービスをアンインストールしたり `docker compose down -v` を実行したりしないでください。
...
1. `.env`、データベース、`data/`、`postgres_data/`、`redis_data/`（または名前付きボリューム）をバックアップします。
2. Compose のアプリケーションイメージを `ghcr.io/heartease1/aipc:<tag>` に変更し、既存のデータベース認証情報とボリューム名は維持します。
3. `docker compose config` で確認してから `docker compose pull` と `docker compose up -d` を実行します。
4. `docker compose ps` とアプリケーションログを確認します。ロールバックする場合は以前のイメージタグに戻し、`docker compose up -d` を再実行します。

移行で置き換わるのはアプリケーションのバイナリまたはイメージだけです。ユーザー、残高、注文、API キー、使用量ログ、サブスクリプションなどのデータは既存のストレージに残ります。
