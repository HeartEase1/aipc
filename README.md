...
[![Release](https://img.shields.io/github/v/release/HeartEase1/aipc?label=AIPC)](https://github.com/HeartEase1/aipc/releases/latest)
[![CI](https://github.com/HeartEase1/aipc/actions/workflows/backend-ci.yml/badge.svg)](https://github.com/HeartEase1/aipc/actions/workflows/backend-ci.yml)
[![License](https://img.shields.io/badge/License-LGPL--3.0-blue.svg)](LICENSE)

[Latest Release](https://github.com/HeartEase1/aipc/releases/latest) · [Source](https://github.com/HeartEase1/aipc) · [Upstream Sub2API](https://github.com/Wei-Shaw/sub2api)

**An independently maintained Sub2API fork for reliable AI API distribution**
...
AIPC is an independent, community-maintained fork of [Sub2API](https://github.com/Wei-Shaw/sub2api). It is not affiliated with, endorsed by, or operated by the upstream project. The original project remains the reference for the upstream architecture and license.

This fork keeps the upstream-compatible API gateway model while extending the operator and user experience with features that are useful in day-to-day deployments:

- **Modern console UI** - A polished modern console with a one-click fallback to the classic console; both modes share the same routes, permissions, API contracts, and business logic.
- **Online playground** - Authenticated browser-based model conversations and image generation using the site's API keys.
- **Leaderboard and benefits** - Usage, spending, and referral leaderboards plus auditable administrator welfare and compensation grants.
- **Promotion controls** - Time-window discounts, participation controls, and region access policies managed from the administrator console.
- **Operational visibility** - Usage statistics, cache hit rates, per-request output speed, channel monitoring, and detailed audit records.
- **Selected compatibility fixes** - Carefully reviewed upstream fixes for OpenAI-compatible request handling, account scheduling, failover, and provider compatibility.

### Maintenance and compatibility

- AIPC follows upstream gateway changes and resolves compatibility with its own features during integration. Each release is an AIPC release, not an official Sub2API release.
- Fork-specific behavior is documented in release notes and the relevant guides. API compatibility, billing boundaries, and data migrations are treated as separate review items.
- The safest upgrade path is to back up the database and configuration, review the release notes, and then update through the release channel configured for your deployment.


## Overview

AIPC is an AI API gateway platform designed to distribute and manage API quotas from AI product subscriptions. Users can access upstream AI services through platform-generated API Keys, while the platform handles authentication, billing, load balancing, and request forwarding.
...
- **Modern and classic consoles** - Choose the modern AIPC console or retain the classic Sub2API-style interface
- **Online Playground** - Use configured API keys for browser-based conversations and image generation
- **Leaderboards and Benefits** - Usage, spending, referral rankings, welfare grants, and compensation records
- **Scheduled Discounts** - Apply time-window promotions to eligible balance-based requests
- **Usage Insights** - Cache hit rates, output speed, request details, and operational monitoring
...
> **AIPC deployment note:** The installer, Docker preparation script, and default application image now use the AIPC release channel. Existing installations can update through the WebUI or the [AIPC Releases](https://github.com/HeartEase1/aipc/releases) page. Runtime compatibility names such as `sub2api.service`, `/opt/sub2api`, and the `sub2api` container are intentionally preserved.

### Migrating from official Sub2API

This fork keeps the existing database schema, configuration paths, service name, and data volumes so an official installation can be moved in place. Back up first, and never uninstall the existing service or run `docker compose down -v` during the migration.

**Script installation:**

```bash
sudo systemctl stop sub2api
sudo cp -a /opt/sub2api "/opt/sub2api.backup.$(date +%Y%m%d%H%M%S)"
sudo cp -a /etc/sub2api "/etc/sub2api.backup.$(date +%Y%m%d%H%M%S)"
curl -fsSL https://raw.githubusercontent.com/HeartEase1/aipc/main/deploy/install.sh | sudo bash -s -- upgrade
...
1. Back up `.env`, the database, and the `data/`, `postgres_data/`, and `redis_data/` directories (or named volumes).
2. Replace the application image with `ghcr.io/heartease1/aipc:<tag>` in your Compose file. Keep the existing database credentials and volume names.
3. Run `docker compose config`, then `docker compose pull` and `docker compose up -d`.
4. Check `docker compose ps` and the application logs. To roll back, restore the previous image tag and run `docker compose up -d` again.

The migration replaces only the application binary/image. Users, balances, orders, API keys, usage logs, subscriptions, and other database data remain in the existing storage.
...
```bash
curl -sSL https://raw.githubusercontent.com/HeartEase1/aipc/main/deploy/install.sh | sudo bash
```
...
# Uninstall
curl -sSL https://raw.githubusercontent.com/HeartEase1/aipc/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
curl -sSL https://raw.githubusercontent.com/HeartEase1/aipc/main/deploy/docker-deploy.sh | bash
...
<a href="https://star-history.dera.page/#HeartEase1/aipc&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=HeartEase1/aipc&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=HeartEase1/aipc&type=Date" />
   <img alt="Star History Chart" src="https://star-history.dera.page/svg?repos=HeartEase1/aipc&type=Date" />
 </picture>
</a>
