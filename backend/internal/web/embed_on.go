...
func (s *FrontendServer) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if (isHostedPlaygroundDocumentPath(path) || isHostedPlaygroundAssetPath(path)) && !s.isOnlinePlaygroundEnabled() {
...
func isHostedPlaygroundAssetPath(cleanPath string) bool {
	cleanPath = strings.TrimPrefix(strings.TrimSpace(cleanPath), "/")
	return strings.HasPrefix(cleanPath, "playground-app/") &&
		!isHostedPlaygroundDocumentPath(cleanPath)
}
