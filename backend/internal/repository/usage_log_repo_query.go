...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		discountCampaignID        sql.NullInt64
		discountFactor            sql.NullFloat64
		originalRateMultiplier    sql.NullFloat64
	...
	log := &service.UsageLog{
		...
		DiscountFactor:            nullFloat64Ptr(discountFactor),
		OriginalRateMultiplier:    nullFloat64Ptr(originalRateMultiplier),
	...
	if discountCampaignID.Valid {
		value := discountCampaignID.Int64
		log.DiscountCampaignID = &value
	}
	if durationMs.Valid {
