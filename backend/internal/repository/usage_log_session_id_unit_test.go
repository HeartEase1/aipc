...
// is immediately before native_compaction_v2.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	...
	// Resolve by column name so later AIPC audit fields cannot shift the assertion.
	sessionArg := prepared.args[usageLogArgumentIndex(t, "session_id")]
	...
	require.Equal(t, "text", usageLogInsertArgTypes[usageLogArgumentIndex(t, "session_id")],
...
func TestPrepareUsageLogInsert_SessionIDNullWhenAbsent(t *testing.T) {
	...
	nsEmpty := preparedEmpty.args[usageLogArgumentIndex(t, "session_id")].(sql.NullString)
