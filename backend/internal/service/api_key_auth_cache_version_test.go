...
func TestAPIKeyService_RejectsV21AuthSnapshotWithoutBlockedModels(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-blocked-models", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v21 auth snapshot to be rejected after blocked models were added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
