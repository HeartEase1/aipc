...
func (s *OpenAIGatewayService) fetchOpenAIImageURLBase64(ctx context.Context, account *Account, rawURL string) (string, error) {
	if strings.HasPrefix(strings.ToLower(rawURL), "data:") {
		if int64(len(rawURL)) > (openAIImageMaxDownloadBytes+2)/3*4+1024 {
			return "", errors.New("data url exceeds image size limit")
		}
		...
		return "", errors.New("data url payload is not an allowed image")
	}
