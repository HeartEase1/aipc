...
// Preserve the first successful provider close. Some providers reject a second
// close, and restarting this timestamp would postpone release indefinitely.
func (s *PaymentService) confirmRechargeReservationClosed(ctx context.Context, orderID int64) error {
	if s.sqlDB == nil {
		return nil
	}
	_, err := s.sqlDB.ExecContext(ctx, `UPDATE recharge_promotion_claims SET closed_confirmed_at=NOW() WHERE order_id=$1 AND status='reserved' AND closed_confirmed_at IS NULL`, orderID)
	return err
}
...
func (s *PaymentService) reconcileRechargeReservations(ctx context.Context) error {
	...
	for _, item := range items {
		...
		if item.closed == nil {
			...
			if err := s.confirmRechargeReservationClosed(ctx, item.id); err != nil {
				return err
			}
			continue
		}
