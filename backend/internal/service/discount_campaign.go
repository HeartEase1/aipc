...
func NewDiscountCampaignService(db *sql.DB) *DiscountCampaignService {
	return &DiscountCampaignService{db: db, stop: make(chan struct{}), done: make(chan struct{})}
}
...
func (s *DiscountCampaignService) Stop() {
	if s == nil {
		return
	}
	...
	defaultDiscountCampaignService.CompareAndSwap(s, nil)
}
