...
func TestGetWebAccessRegionSettingsDefaultsDisabled(t *testing.T) {
	svc := &SettingService{settingRepo: &webAccessSettingRepoStub{getErr: ErrSettingNotFound}}
	settings, err := svc.GetWebAccessRegionSettings(context.Background())
	require.NoError(t, err)
	require.False(t, settings.BlockMainlandChina)
}

func TestGetWebAccessRegionSettingsEnabled(t *testing.T) {
	svc := &SettingService{settingRepo: &webAccessSettingRepoStub{value: "true"}}
	settings, err := svc.GetWebAccessRegionSettings(context.Background())
	require.NoError(t, err)
...
func TestSetWebAccessRegionSettingsPersistsAndRefreshes(t *testing.T) {
	...
	refreshes := 0
	svc.SetOnUpdateCallback(func() { refreshes++ })
	err := svc.SetWebAccessRegionSettings(context.Background(), &WebAccessRegionSettings{BlockMainlandChina: true})
	require.NoError(t, err)
...
func TestIsOnlinePlaygroundEnabledDefaultsEnabledWhenSettingIsMissing(t *testing.T) {
	svc := &SettingService{settingRepo: &webAccessSettingRepoStub{getErr: ErrSettingNotFound}}
	enabled, err := svc.IsOnlinePlaygroundEnabled(context.Background())
	require.NoError(t, err)
	require.True(t, enabled)
}

func TestIsOnlinePlaygroundEnabledHonorsExplicitDisable(t *testing.T) {
	svc := &SettingService{settingRepo: &webAccessSettingRepoStub{value: "false"}}
	enabled, err := svc.IsOnlinePlaygroundEnabled(context.Background())
	require.NoError(t, err)
...
func TestIsOnlinePlaygroundEnabledReturnsStorageErrors(t *testing.T) {
	svc := &SettingService{settingRepo: &webAccessSettingRepoStub{getErr: errors.New("database unavailable")}}
	enabled, err := svc.IsOnlinePlaygroundEnabled(context.Background())
	require.ErrorContains(t, err, "database unavailable")
	require.False(t, enabled)
}
