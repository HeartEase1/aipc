...
// 系统日志保留天数由 ops_runtime_log_config 拥有，不再跟随错误日志天数。
...
// 两个 key 独立 fallback：runtime log config 坏了不能把 advanced settings 的覆盖一起丢掉。
...
// 部分字段的 advanced settings 走安全默认值，而不是 Go 零值（0 = 清空全部）。
