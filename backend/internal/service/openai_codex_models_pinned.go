...
func (s *OpenAIGatewayService) FetchPinnedCodexModelsManifest(ctx context.Context, group *Group, clientVersion string) (*CodexModelsManifest, *Account, error) {
	...
	if len(cfg.AccountIDs) > maxCodexModelsManifestAccounts {
		return nil, nil, infraerrors.New(http.StatusBadRequest, "INVALID_CODEX_MODELS_MANIFEST_CONFIG", "too many pinned codex models manifest accounts")
	}
	...
	for i := range usable {
		...
		go func() {
			...
			if completeErr := s.CompleteAPIKeyCodexModelsManifestForClient(manifest, &usable[index]); completeErr != nil {
