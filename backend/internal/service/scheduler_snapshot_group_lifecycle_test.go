...
func TestSchedulerGroupLifecycleActiveReopensAndRebuildsAllCurrentBuckets(t *testing.T) {
	...
	accounts.beforeLoad = func() {
		...
		require.Equal(t, schedulerCanonicalCount(), tokenCount, "all reopen tokens must be prepared before the first account query")
	}
	svc := newGroupLifecycleTestService(cache, accounts, groups, config.RunModeStandard)
	seen := make(map[batchSeenKey]struct{})
	...
	require.Len(t, cache.tokens(), schedulerCanonicalCount())
	...
	require.Len(t, reopenHeld, schedulerCanonicalCount())
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	lockTTLs, unlockCalls := cache.lockStats()
	require.Len(t, lockTTLs, schedulerCanonicalCount())
	for _, ttl := range lockTTLs {
		require.Equal(t, 30*time.Second, ttl)
	}
	require.Equal(t, schedulerCanonicalCount(), unlockCalls)
	requireLifecycleSeen(t, seen, groupID)
}
...
func TestSchedulerGroupLifecycleEpochPreventsABA(t *testing.T) {
	...
	require.Len(t, firstActiveTokens, schedulerCanonicalCount())
	...
	require.Len(t, allTokens, 2*schedulerCanonicalCount())
	require.Greater(t, allTokens[schedulerCanonicalCount()].Epoch, firstActiveTokens[0].Epoch)
