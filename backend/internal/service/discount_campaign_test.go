...
func TestResolveOpenAIUsageTokenDiscountExcludesVideoEvenWithTokenPricing(t *testing.T) {
	...
	cost := &CostBreakdown{BillingMode: string(BillingModeToken), TotalCost: 5, ActualCost: 10}
	require.Nil(t, applyOpenAITokenMarketingDiscount(group, 1, video, []string{"grok-imagine-video"}, now, 2, cost, 0))
	require.Equal(t, float64(10), cost.ActualCost)

	text := &OpenAIForwardResult{Model: "gpt-5"}
	resolved := applyOpenAITokenMarketingDiscount(group, 1, text, []string{"gpt-5"}, now, 2, cost, 0)
	require.NotNil(t, resolved)
	require.InDelta(t, 1.8, resolved.EffectiveRateMultiplier, 1e-9)
	require.InDelta(t, 9, cost.ActualCost, 1e-9)
	require.Equal(t, float64(5), cost.TotalCost)
}
