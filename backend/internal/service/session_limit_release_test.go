...
func (s *sessionLimitReleaseCacheStub) RegisterSessionLease(context.Context, int64, string, int, time.Duration, string) (bool, bool, error) {
	return true, true, s.err
}

func (s *sessionLimitReleaseCacheStub) ReleaseSessionLease(ctx context.Context, accountID int64, sessionID, token string) error {
	if token == "" {
	...
	return s.UnregisterSession(ctx, accountID, sessionID)
}
...
func TestReleaseAccountSession_ReleasesRegisteredSlot(t *testing.T) {
	...
	ctx, finish := svc.WithSessionRegistrationTracking(context.Background())
	defer finish()
	require.True(t, svc.checkAndRegisterSession(ctx, acc, "session-hash-1"))
	svc.ReleaseAccountSession(ctx, acc, "session-hash-1")
...
func TestReleaseAccountSession_Idempotent(t *testing.T) {
	...
	svc.ReleaseAccountSession(ctx, acc, "session-hash")
	svc.ReleaseAccountSession(ctx, acc, "session-hash")

	require.Len(t, cache.unregistered[42], 1)
}

func TestSessionLeaseCleanupPreservesSuccessAndUntrackedSessions(t *testing.T) {
	cache := newSessionLimitReleaseCacheStub()
	svc := &GatewayService{sessionLimitCache: cache}
	account := newSessionLimitTestAccount()
	ctx, finish := svc.WithSessionRegistrationTracking(context.Background())
	require.True(t, svc.checkAndRegisterSession(ctx, account, "before-admission-failure"))
	require.True(t, svc.checkAndRegisterSession(ctx, account, "served"))
	svc.KeepAccountSession(ctx, account, "served")
	svc.ReleaseAccountSession(context.Background(), account, "untracked")
	finish()
	require.Equal(t, []string{"before-admission-failure"}, cache.unregistered[account.ID])
}
