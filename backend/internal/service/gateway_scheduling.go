...
func (s *GatewayService) checkAndRegisterSession(ctx context.Context, account *Account, sessionID string) bool {
	...
	if tracker := sessionLeases(ctx); tracker != nil {
		if cache, ok := s.sessionLimitCache.(sessionLimitLeaseCache); ok {
			allowed, owned, err := cache.RegisterSessionLease(ctx, account.ID, sessionID, maxSessions, idleTimeout, tracker.token)
			if err != nil {
				return true
			}
			if owned {
				tracker.mu.Lock()
				tracker.accounts[sessionLeaseKey{account.ID, sessionID}] = struct{}{}
				tracker.mu.Unlock()
			}
			return allowed
		}
...
// ReleaseAccountSession releases only registrations still owned by this request.
// Existing sessions and sessions reused by another request retain their idle TTL.
func (s *GatewayService) ReleaseAccountSession(ctx context.Context, account *Account, sessionID string) {
	...
	tracker := sessionLeases(ctx)
	cache, ok := s.sessionLimitCache.(sessionLimitLeaseCache)
	if tracker == nil || !ok {
		return
	}
	key := sessionLeaseKey{account.ID, sessionID}
	tracker.mu.Lock()
	_, owned := tracker.accounts[key]
	tracker.mu.Unlock()
	if !owned {
		return
	}
	...
	defer cancel()
	if err := cache.ReleaseSessionLease(cleanupCtx, account.ID, sessionID, tracker.token); err != nil {
		slog.Debug("session_limit.release_failed",
			"account_id", account.ID,
			"error", err)
		return
	}
	s.KeepAccountSession(ctx, account, sessionID)
}
