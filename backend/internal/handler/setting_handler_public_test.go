...
func TestSettingHandler_GetPublicSettings_ExposesRechargeMultiplier(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerPublicRepoStub{
		values: map[string]string{
			service.SettingPaymentEnabled:      "true",
			service.SettingBalanceRechargeMult: "7.5",
		},
	}
	h := NewSettingHandler(service.NewSettingService(repo, &config.Config{}), "test-version")
	recorder := httptest.NewRecorder()
	...
	require.Equal(t, 7.5, resp.Data.PaymentBalanceRechargeMultiplier)
}

func TestSettingHandler_GetPublicSettings_ExposesOnlinePlaygroundEnabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerPublicRepoStub{values: map[string]string{
		service.SettingKeyOnlinePlaygroundEnabled: "false",
	}}
	h := NewSettingHandler(service.NewSettingService(repo, &config.Config{}), "test-version")
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.False(t, resp.Data.OnlinePlaygroundEnabled)
}

func TestSettingHandler_GetPublicSettings_ExposesConsoleUIMode(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerPublicRepoStub{values: map[string]string{
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.Equal(t, service.ConsoleUIModeModern, resp.Data.ConsoleUIMode)
}
