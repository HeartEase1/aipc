...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if effectiveAPIKeyPlatform(c, apiKey) == service.PlatformAnthropic {
		if policyBody, changed := applyOpenAIReasoningEffortPolicyForRequest(c, apiKey, body); changed {
			if err := parsedReq.ReplaceBody(policyBody); err != nil {
				h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "Invalid reasoning effort policy result")
				return
			}
			body = parsedReq.Body.Bytes()
		}
	...
	// 失败时仅释放本请求新建且仍独占的会话注册；并发复用及成功会话保持空闲超时。
	sessionSlotAccounts := make(map[int64]*service.Account)
	leaseCtx, finishSessionLeases := h.gatewayService.WithSessionRegistrationTracking(c.Request.Context())
	c.Request = c.Request.WithContext(leaseCtx)
	defer finishSessionLeases()
	upstreamServedSession := false
	defer func() {
		if upstreamServedSession {
			return
		}
		// ReleaseAccountSession 保留凭据，并在内部脱离取消信号后限时释放。
		for _, acc := range sessionSlotAccounts {
			h.gatewayService.ReleaseAccountSession(c.Request.Context(), acc, sessionKey)
		}
	}()
	...
	for {
		...
		for {
			...
			if vetoed {
				...
				h.gatewayService.ReleaseAccountSession(c.Request.Context(), account, sessionKey)
			...
			if err != nil {
				...
				if result != nil {
					...
					h.gatewayService.KeepAccountSession(c.Request.Context(), account, sessionKey)
				}
				return
			}
...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	h.gatewayService.KeepAccountSession(c.Request.Context(), account, sessionHash)
}
