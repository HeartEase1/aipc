...
func (h *APIKeyHandler) GetAvailableGroups(c *gin.Context) {
	...
	for i := range groups {
		...
		if discount := service.ResolveTokenDiscountForUser(&groups[i], subject.UserID, time.Now(), baseRate); discount != nil {
			item.EffectiveRateMultiplier = discount.EffectiveRateMultiplier
			item.DiscountCampaignID = &discount.CampaignID
			item.DiscountCampaignName = discount.CampaignName
			item.DiscountCampaignDescription = discount.CampaignDescription
			item.DiscountFactor = &discount.DiscountFactor
