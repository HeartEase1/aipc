...
[最新版本](https://github.com/HeartEase1/aipc/releases/latest) · [源码仓库](https://github.com/HeartEase1/aipc) · [上游 Sub2API](https://github.com/Wei-Shaw/sub2api)

**独立维护的 Sub2API 分支：更完整的 AI API 分发与管理控制台**
...
AIPC 是基于 [Sub2API](https://github.com/Wei-Shaw/sub2api) 的独立社区维护分支，与上游项目不存在隶属、授权或官方背书关系。原项目仍是上游架构与许可证的参考来源。

本分支保留上游兼容的 API 网关模式，并针对实际部署补充了以下管理与用户体验能力：

- **现代控制台** - 提供统一设计的现代控制台，同时保留经典控制台作为一键回退选项；两种模式共用路由、权限、API 契约和业务逻辑。
- **在线工作台** - 登录后使用站点 API Key 在线对话和生图。
- **排行榜与福利** - 使用榜、消费榜、返利榜，以及可审计的管理员福利与补偿发放记录。
- **活动与访问控制** - 限时折扣、排行榜参与开关和地区访问策略均可在管理后台配置。
- **运维可见性** - 用量统计、缓存命中率、单请求输出速率、渠道监控和操作审计记录。
- **精选兼容修复** - 对上游 OpenAI 兼容请求、账号调度、故障转移和供应商协议修复进行风险评估后选择性纳入。

### 维护与兼容策略

- AIPC 跟随上游网关更新，并在整合时处理与自有功能的兼容。每个版本都是 AIPC 版本，不代表官方 Sub2API 版本。
- 分支特有行为会在 Release 说明和对应指南中记录；API 兼容性、计费边界和数据库迁移会作为独立项目审查。
- 升级前建议备份数据库与配置，阅读版本说明，再通过当前部署配置的发布渠道更新。


## 项目概述

AIPC 是一个 AI API 网关平台，用于分发和管理 AI 产品订阅的 API 配额。用户通过平台生成的 API Key 调用上游 AI 服务，平台负责鉴权、计费、负载均衡和请求转发。
...
- **管理后台** - Web 界面进行监控和管理
- **现代与经典双控制台** - 可使用现代 AIPC 控制台，也可切回经典 Sub2API 风格界面
- **在线工作台** - 使用已配置的 API Key 在线对话和生图
- **排行榜与福利** - 使用、消费、返利排行，以及福利与补偿发放记录
- **限时折扣活动** - 对符合条件的余额请求应用指定时间段的优惠倍率
- **用量分析** - 缓存命中率、单请求输出速率、请求明细和渠道运行监控
...
> **AIPC 部署说明：** 安装脚本、Docker 准备脚本和默认应用镜像现在均使用 AIPC 发布渠道。已有实例可以通过 WebUI 或 [AIPC Releases](https://github.com/HeartEase1/aipc/releases) 更新。为兼容原有部署，`sub2api.service`、`/opt/sub2api` 和 `sub2api` 容器名会继续保留。

### 从官方 Sub2API 切换到 AIPC

本分支保留现有数据库结构、配置路径、服务名和数据卷，可以在原地切换。请先备份，切换过程中不要卸载原服务，也不要执行 `docker compose down -v`。
...
1. 备份 `.env`、数据库，以及 `data/`、`postgres_data/`、`redis_data/` 目录（或命名卷）。
2. 将 Compose 中的应用镜像改为 `ghcr.io/heartease1/aipc:<tag>`，保留原有数据库凭证和卷名。
3. 先执行 `docker compose config`，再执行 `docker compose pull` 和 `docker compose up -d`。
4. 检查 `docker compose ps` 和应用日志。需要回滚时恢复原镜像标签，再次执行 `docker compose up -d`。

切换只会替换应用二进制或镜像，用户、余额、订单、API Key、用量日志、订阅和其他数据库数据仍保存在原有存储中。
