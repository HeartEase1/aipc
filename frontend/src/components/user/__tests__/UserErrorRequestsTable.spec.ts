...
describe('UserErrorRequestsTable viewport', () => {
  it('uses a bounded table window only when requested', () => {
    const regular = mountTable()
    const bounded = mountTable(true)
    expect(regular.get('.user-error-requests-table').classes())
      .not.toContain('user-error-requests-table--fixed-viewport')
    expect(bounded.get('.user-error-requests-table').classes())
