<template>
  ...
  >
    <div class="card relative z-10 overflow-visible p-4 flex items-center gap-3">
    ...
    <div class="card relative z-10 overflow-visible p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-gray-500">
          ...
          <span>/</span>
          <UsageInfoPopover :label="cacheDetailLabel()">
            <template #trigger><span>{{ cacheLabel() }}: {{ formatTokens(stats?.total_cache_tokens || 0) }} ⓘ</span></template>
...
import UsageInfoPopover from '@/components/common/UsageInfoPopover.vue'
