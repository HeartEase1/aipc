<template>
  <div class="mt-4 space-y-3 border-t border-gray-200 pt-3 text-sm dark:border-dark-600">
    ...
    <template v-if="summary.enabled">
      ...
      <p class="text-xs text-gray-500 dark:text-gray-400">{{ summary.next_tier ? t('balanceMarketing.nextTier', { amount: amountOnly(summary.amount_to_next || '0'), tier: summary.next_tier }) : t('balanceMarketing.maxTier') }}</p>
    ...
    <BaseDialog :show="showRules" :title="t('balanceMarketing.rules')" width="normal" @close="showRules = false">
