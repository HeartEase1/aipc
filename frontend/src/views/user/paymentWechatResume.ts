...
export function parseWechatResumeRoute(
  ...
  const subscriptionAction = readQueryString(query, 'subscription_action') === 'restart' ? 'restart' : 'extend'
  ...
  if (wechatResumeToken) {
    return {
      ...
      ...(orderType === 'subscription' ? { subscriptionAction } : {}),
    }
  }
...
export function stripWechatResumeQuery(query: LocationQuery): LocationQueryRaw {
  ...
  delete nextQuery.subscription_action
  return nextQuery
}
