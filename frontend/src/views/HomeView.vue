      ...
      <nav class="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <router-link
            v-if="showModelPlazaEntry"
            to="/model-plaza"
            class="inline-flex items-center gap-1.5 rounded-lg p-2 text-sm text-white/40 transition-colors hover:bg-white/5 hover:text-white/80"
            :title="t('nav.modelPlaza')"
          >
            <Icon name="grid" size="md" />
            <span class="hidden sm:inline">{{ t('nav.modelPlaza') }}</span>
...
const siteName = computed(() => appStore.cachedPublicSettings?.site_name || appStore.siteName || 'AIPC')
...
const paymentEnabled = computed(() =>
  ...
  appStore.cachedPublicSettings?.payment_balance_disabled !== true
)
