export default {
    accounts: {
      ...
      openai: {
        ...
        codexCLIOnlyDesc: '适用于 OpenAI OAuth 和 API Key 账号。开启后仅允许已通过身份与指纹校验的 Codex 官方客户端访问；关闭后保持原有访问逻辑。',
      ...
      pelicanTest: {
        ...
        model: '模型',
        modelHint: '搜索账号可用模型，或输入自定义模型 ID；优先选择 gpt-6-astra。',
        defaultPrompt: '创建一个独立 HTML，用 SVG 绘制鹈鹕骑自行车的 2D 动画。',
        deliveryContract: '直接返回完整独立 HTML，不使用 Markdown 代码块或外部依赖。只输出 HTML，不要解释。预览在隔离沙箱中运行，限制外部资源加载。',
        reasoningDefault: '模型默认（不传思考参数）',
        reasoningHint: '按账号模型映射和能力显示可用档位。Ultra 属于客户端多任务模式，不用于单次生成。',
        effort: { none: '关闭（none）', minimal: '极低（minimal）', low: '低（low）', medium: '中（medium）', high: '高（high）', xhigh: '超高（xhigh）', max: '最高（max）' },
        clearHistory: '清除此账号记录',
        stopAndClose: '停止并关闭',
        incompleteResponse: '响应流未正常完成，请重试；已收到的原文保留在下方。',
        reasoning: '思考强度',
        reasoningLow: '低',
        reasoningMedium: '中',
        reasoningHigh: '高',
        parallel: '并行数',
        parallelHint: '同一账号最多并行 8 份，每份独立消耗上游额度。浏览器按管理员保存最近 8 次记录，总容量最多约 4 MB。',
        results: '新建测试',
        history: '测试记录',
