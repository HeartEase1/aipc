export default {
  ...
  onlinePlayground: {
    title: '在线使用',
    description: '直接使用你的站内 API 密钥进行多轮对话、图片生成与图片编辑。',
    configuration: '工作台配置',
    textChat: '多窗口对话',
    imageGeneration: '在线生图',
    selectKey: 'API 密钥',
    selectKeyPlaceholder: '选择一个可用密钥',
    unnamedKey: '未命名密钥',
    selectTextModel: '对话模型',
    selectImageModel: '生图模型',
    selectModelPlaceholder: '选择模型',
    base64Images: '返回 Base64 图片数据',
    base64ImagesHint: '开启后优先请求内嵌图片数据，避免部分图片链接的跨域限制；不支持的服务商会自动保留原始链接供复制。',
    base64ImagesOn: '已开启 Base64 图片数据',
    base64ImagesOff: '已关闭，使用服务商返回的图片链接',
    keyPrivacyNotice: '保存配置只会在当前浏览器记录密钥编号、模型和图片偏好，不会保存密钥内容。密钥内容仅在当前页面内存中使用，不会出现在地址栏、聊天记录或本地历史中。所有请求仍通过本站网关并按现有规则计费；Agent 会按任务需要自动发起多次计费请求，单次操作最多 6 轮对话请求和 6 个生图请求。',
    loadingKeys: '正在加载可用密钥…',
    loadingModels: '正在获取模型列表…',
    keysLoadFailed: '无法加载 API 密钥，请检查网络后重试。',
    modelsLoadFailed: '无法使用所选密钥获取模型列表。请确认密钥、分组和 IP 限制可用。',
    httpStatus: '请求状态码：{status}',
    noActiveKey: '暂无可用的 API 密钥',
    createKeyHint: '请先创建并启用一个密钥，再进入在线工作台。',
    goToKeys: '前往创建密钥',
    saveConfiguration: '保存配置',
    configurationSaved: '配置已保存',
    saveConfigurationFailed: '浏览器未能保存配置，请检查是否禁用了网站存储。',
    refreshModels: '刷新模型',
    retry: '重试',
    workspace: '在线创作工作台',
    iframeTitle: '在线对话与生图工作台',
    connecting: '工作台连接中…',
    connected: '工作台已连接',
    connectionFailed: '工作台连接失败',
    connectionFailedHint: '工作台静态资源可能尚未构建或连接已超时，请重新连接。',
    reconnect: '重新连接',
    enterFullscreen: '全屏使用工作台',
    exitFullscreen: '退出全屏',
    localHistoryNotice: '对话、任务和图片历史仅保存在当前浏览器，并按账号使用独立存储；本站不会同步到云端。共享设备上的其他使用者可能读取浏览器本地数据，请在使用后清理本站浏览器数据。'
  },
  ...
  leaderboard: {
    ...
    consumptionTab: '消费榜',
    rebateTab: '返利榜',
    summaryScope: '汇总数据为当前时间范围内所有参与排行榜用户的数据合计。',
    usageRule: '按消耗 Token 降序排列，表中数据均为所选时间范围内合计。',
    consumptionRule: '按实际消耗额度降序排列，表中数据均为所选时间范围内合计。',
    rebateRule: '按返利金额降序排列，邀请用户为所选时间范围内新增绑定人数。',
    shareOverview: '总量占比',
    usageShareDescription: '按消耗 Token 计算，展示前 20 名和其他参与用户占总 Token 的比例。',
    consumptionShareDescription: '按消耗额度计算，展示前 20 名和其他参与用户占总消耗额度的比例。',
    rebateShareDescription: '按返利金额计算，展示前 20 名和其他参与用户占总返利金额的比例。',
    otherUsers: '其他人',
    top20: '前 20 名',
    myData: '我的数据',
    totalRequests: '总请求数',
    totalTokens: '总 Token',
    totalCost: '总消耗额度',
    totalInvitedUsers: '总邀请用户',
    totalRebateCount: '总返利笔数',
    totalRebateAmount: '总返利金额',
    rank: '排名',
