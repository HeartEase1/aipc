export default {
  ...
  keys: {
    ...
    fastMode: 'Fast 模式',
    fastModeHint: '开启后，此密钥的 OpenAI 请求会请求 Priority 档位，仍受全局 Fast/Flex 策略控制。费用按实际档位及分组定价规则计算；非 OpenAI 分组不生效。',
    ipRestriction: 'IP 限制',
    ipWhitelist: 'IP 白名单',
  ...
  usage: {
    ...
    latencyDuration: '总耗时',
    outputRate: '输出速率',
    outputRateDetails: '输出速率计算说明',
    outputRateAverage: '平均输出速率（含首字等待）',
    outputRateGeneration: '生成速率（不含首字等待）',
    outputRateAverageFormula: '输出 Token ÷ 总耗时，从请求开始计算',
    outputRateGenerationFormula: '输出 Token ÷ 生成耗时，从首个输出开始计算',
    outputRateGenerationWarning: '生成速率更接近模型开始回复后的输出速度，但会受上游计时和数据缓冲影响，仅供参考。',
    time: '时间',
    ws: 'WS',
    stream: '流式',
    sync: '同步',
