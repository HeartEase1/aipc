export default {
  discountCampaigns: {
    ...
    fields: {
      name: 'キャンペーン名', description: 'ユーザー向け説明', descriptionPlaceholder: '割引のルールや説明を入力...', groupScope: '適用範囲', applicableGroups: '適用グループ', enabled: 'キャンペーンを有効化', scheduleType: 'スケジュール', timezone: 'タイムゾーン',
    ...
    groupScopes: { all: 'すべての残高グループ', specific: '指定した残高グループ', selectedCount: '残高グループを {count} 件指定' },
    ...
    hints: {
      ...
      groupScope: '選択した通常の残高グループだけが対象です。サブスクリプショングループは常に対象外です。',
      allBalanceGroups: 'すべての通常残高グループに適用'
    },
    ...
    errors: { load: '割引キャンペーンを読み込めませんでした', loadGroups: 'グループを読み込めませんでした', groupRequired: '適用グループを1つ以上選択してください', save: '割引キャンペーンを保存できませんでした', delete: '割引キャンペーンを削除できませんでした' }
  }
}
